...
- **Updated:** 2026-08-22 (maintainer run 32558534766, owner `/oc maintainer` on PR #118). Fresh survey confirms: PR #118 head `961fa2a` (B5.31, 11.059 bpp, byte-exact, harness ~345s), **no `continue` build was in flight** when this run started (last real build `32557569730` COMPLETED landing B5.31). This run has therefore dispatched a fresh owner-directed `continue` (head `961fa2a`) to resume B6-B8 - B7 is now mandatory. `opencode.yml` STILL has NO `lab` job and NO `workflows: write` permission; reviewer.md auto-guard (commit 770a756) still enforces infra routing. #119/#120 CLOSED by owner.
- **Maintainer run 32558534766 (07:02Z)** dispatched the owner-directed `continue` and wrote the public comment emphasizing B7. A prior scheduled run (32558119663, 06:53Z) had only re-confirmed the then-in-flight build; this run found the loop paused again at B5.31 and resumed it.
...
- **`main` = `770a7567c147fbd00373691c7a59d8000f992b87`** (last commit: "reviewer: route infrastructure PRs to the Lab Engineer and enforce it"). Obsidian lives in `obsidian/` on `main`. Prism branch `opencode/117-prism-m1-m4-optimization` = `961fa2a` shares M0 ancestry (NOT orphan).
...
- **CIRCUIT-BREAKER FALSE-TRIP (still pending owner action):** now at 32/20 false trips. The "no converging / net-negative" premise (Obsidian #93) is FALSE for Prism - steady genuine byte reductions (11.29 -> 11.059, ~2% total). Owner has been manually re-authorizing each iteration; #120 closed means the budget was NOT raised via that issue, so the manual re-auth pattern continues. Please raise the breaker budget or repivot/close #117.

## IN FLIGHT
- **Prism M1-M4 (issue #117, PR #118, branch `opencode/117-prism-m1-m4-optimization`):** head `961fa2a` (B5.31, 11.059 bpp, byte-exact, harness ~345s). **As of this run: NO build was in flight** (last real build `32557569730` completed landing B5.31). This run dispatched a fresh owner-directed `continue` (decision list, head `961fa2a`) to resume B6-B8. **Predictor bank is FULLY EXHAUSTED (16/16 nibble 0..15, per-plane top7, block top9, selective-16 thr40 top10) AND B6 5/3 lifting is done and inert (+0.8% never-expand, kept disabled).** B7 (Squeeze+MA-tree greedy split depth 6 with mandatory `llc_class`/`sibling_class`) is the ONLY proven >10% closure to M3 < 8.71 and MUST be attempted next - no more B5.x/B6 headroom. The dispatched `continue` MUST build B7, not another exhausted-bank tweak.
...
5. **Silent-stall mitigation:** re-dispatch owner-directed `continue` when no build is in flight (fired this run - loop was paused at B5.31).
...
- **Circuit breaker:** false-trip persists (32/20); owner manual re-auth continues; #120 closed without raising budget.
