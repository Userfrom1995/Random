...
- **Updated:** 2026-08-21 (maintainer run 32498502131, event `created` on PR #114, 15:36Z). Owner override at 15:36Z (`/oc lab fix that indentation issue and push`) granted the `workflows: write` path that bot runs lacked. Maintainer applied the `maintainer.yml` indentation fix directly to the PR branch and re-routes to the Reviewer. PR #114 now has NO blocking findings; pending Reviewer approval -> Tester -> merge -> close #112. PR #104 still in review.
...
- **PR #114 indentation blocker - RESOLVED THIS RUN.** `.github/workflows/maintainer.yml` lines 333 + 339 corrected (14-space `elif`, 18-space bodies) to match the dispatch chain. `ast.parse` on the dedented dispatch block now passes; `yaml.safe_load` passes. The fix was applied with owner authorization (`workflows: write`); prior bot runs were all `remote rejected` for lack of that scope. All other findings already clear on head `a05e323` (orphan re-link, free pin, doc syncs, `github.token` recover-tag, em-dash clean).
- Issue #110 CLOSED (via #111 merge).

## IN FLIGHT
1. **PR #104 (Prism, issue #103) - M0 BUILD DONE, IN REVIEW.** Head `4cb0247331296a6a23c928987696d2b4282c086e`. 47 files, +11592/-8, merge-base `0eb9de0` (NO orphan). Routed to Reviewer; reviews=[] still. Unaffected by #114.
2. **PR #114 (lab recovery for #112) - INDENTATION FIX APPLIED, RE-ROUTED TO REVIEWER.** Branch `opencode/issue112-20260821125724` head `a05e323` (re-linked clean, free model, doc synced, IndentationError now fixed). Awaiting `/oc approve` -> Tester -> Mae merge (lab infra) -> close #112.

## PENDING (in order)
1. **PR #114 review -> test -> merge -> close #112.** Lab infra PR, exempt from 2/day new-project limit.
...
- **#112 (automatic PR recovery)** - OPEN; delivered via PR #114 (indentation fix applied this run; re-review pending).
...
- `maintainer.yml` trigger dispatch now FIXED on the PR branch (IndentationError at 333/339 resolved); `/oc recover` can post once #114 merges.

## NEXT STEPS
1. Reviewer approves PR #114 (indentation fixed) -> Tester (recover.sh + opencode-recover.yml dry checks) -> Mae merge (lab infra) -> close #112.
2. Reviewer approves PR #104 (M0) -> Tester (Kodak vs JXL 3.1 bpp) -> Mae merge -> close #103.
3. If Reviewer requests further fixes on either PR, route Fixer (`/oc fix`); note #114 workflow-file fixes now only need `workflows: write` (granted by owner override this run).

## OPEN QUESTIONS
- After #114 approval: will the Tester pass and Mae merge (lab infra) -> close #112?
