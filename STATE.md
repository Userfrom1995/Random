...
- **Updated:** 2026-08-22 (maintainer run 32560156556, owner `/oc maintainer` on PR #118). Fresh survey confirms: PR #118 head `08bf09578a` (B5.33, 11.059 bpp, byte-exact, harness ~375s), **no `continue` build was in flight** when this run started (last real build `32559653918` COMPLETED landing B5.33). This run has therefore dispatched a fresh owner-directed `continue` (head `08bf09578a`) to resume B7 - B7 IS MANDATORY. `opencode.yml` STILL has NO `lab` job and NO `workflows: write` (reviewer.md auto-guard committed 770a756 still enforces infra routing). #119/#120 CLOSED by owner.
- **Maintainer run 32560156556 (07:38Z)** dispatched the owner-directed `continue` and wrote the public comment emphasizing B7 (MA-tree greedy split on top of B5.33 squeeze scaffold). A prior run (32559525132, 07:24Z) had found the loop paused at B5.32 and dispatched a `continue` (head 2e65cef1) which ran and landed B5.33; this run found the loop paused again at B5.33 and resumed it.
...
- **`main` = `770a7567c147fbd00373691c7a59d8000f992b87`** (last commit: "reviewer: route infrastructure PRs to the Lab Engineer and enforce it"). Obsidian lives in `obsidian/` on `main`. Prism branch `opencode/117-prism-m1-m4-optimization` = `08bf09578a` shares M0 ancestry (NOT orphan).
...
- **CIRCUIT-BREAKER FALSE-TRIP (still pending owner action):** now at 34/20 false trips. The "no converging / net-negative" premise (Obsidian #93) is FALSE for Prism - steady genuine byte reductions (11.29 -> 11.059, ~2% total, each a byte-exact Kodak win). Owner has been manually re-authorizing each iteration via `/oc maintainer`; #120 closed means the budget was NOT raised via that issue, so the manual re-auth pattern continues. Please raise the breaker budget or repivot/close #117.

## IN FLIGHT
- **Prism M1-M4 (issue #117, PR #118, branch `opencode/117-prism-m1-m4-optimization`):** head `08bf09578a` (B5.33, 11.059 bpp, byte-exact, harness ~375s). **As of this run: NO build was in flight** (last real build `32559653918` completed landing B5.33). This run dispatched a fresh owner-directed `continue` (decision list, head `08bf09578a`) to resume B6-B8 - B7 is now MANDATORY. **Predictor bank is FULLY SATURATED** (16/16 nibble 0..15, per-plane top9, block top11, selective-16 thr50 top12) per B5.33's own report, B6 5/3 lifting done and inert (+0.8% never-expand, kept disabled), and B5.33 added a **B7 per-band squeeze scaffold** (still +0.8% never-expand, lacks MA-tree `llc_class`/`sibling_class`). B7 (Squeeze+MA-tree greedy split depth 6 with mandatory `llc_class`/`sibling_class`) is the ONLY proven >10% closure to M3 < 8.71 and MUST be attempted next - no more B5.x/B6 headroom. The dispatched `continue` MUST build the MA-tree greedy split, not another exhausted-bank tweak.
...
5. **Silent-stall mitigation:** re-dispatch owner-directed `continue` when no build is in flight (fired this run - loop was paused at B5.33).
...
- **Circuit breaker:** false-trip persists (34/20); owner manual re-auth continues; #120 closed without raising budget.
...
## OPEN QUESTIONS
- Will the Builder actually attempt the B7 MA-tree greedy split (mandatory `llc_class`/`sibling_class`) on the dispatched `continue`, instead of another exhausted-bank B5.x/B6 tweak? This is the only path to M3 < 8.71.
