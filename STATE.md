...
- **Updated:** 2026-08-18 (~16:25Z, maintainer run 32159973855, triggered by owner `/oc maintainer` on PR #83 after the Architect delivered the R4 blueprint). **DECISIONS:** `[{"action":"continue","pr":83}]`. The Architect (run delivering commit `33bd48f`) proved the real root cause is the binary range coder itself (lossless but does NOT compress: 3.7-41x over Shannon bound), and blueprinted R4 (replace it with a correct carryless LZMA range coder + a mandatory efficiency gate). Resuming the Builder via `continue` to implement R4 in isolation, then re-measure R1/R2/R3 on REAL Kodak. No merge (gates unmet). One PR preserved.
...
- **ROOT-CAUSE DIAGNOSIS (now CONVERGED, R4):** The shared 16-bit binary range coder (`RangeEnc`/`RangeDec`/`BinEnc`/`BinDec`) is **lossless but does NOT compress** - it emits ~1 bit/symbol regardless of the learned probability (empirical probe: p=0.1 → 1.745 bps vs 0.469 Shannon = 3.72x; p=0.01 → 3.348 vs 0.081 = 41x). This is the real defect behind every CMARC/R3 "regression": context/quotient/residual tuning is futile because the coder ignores probability. GR is unaffected (separate `GrState` Golomb-Rice coder). The Researcher's "per-bin cost ~1.88 bits" was the *symptom*, the Architect's R4 correctly names the *coder* as the defect.
- **R4 blueprint (Architect, commit `33bd48f`, 16:22Z):** Replace the four broken binary coders with a correct **carryless LZMA range coder** (32-bit `range`, 64-bit `low` accumulator with `ShiftLow` carry cache, `PRECISION=12`/`BIN_TOTAL=4096`, preserving the `BinModel { p: u16 }` interface and `put`/`get` signatures). **MANDATORY efficiency gate:** `measured_bps / shannon_bps < 1.10` for `p ∈ {0.01,0.1,0.5,0.9,0.99}` + Laplacian - fails the build until the coder is correct (round-trip tests cannot catch a lossless-but-non-compressing coder). Build order: R4 in isolation → re-measure R1/R2 → re-measure R3 on REAL Kodak.

## In flight

- **Builder `continue** (this decision):** resume the Builder on the single existing branch to implement R4 (replace the broken binary range coder with the correct carryless LZMA coder), land the mandatory efficiency gate, confirm `cargo test -p obsidian_core` green, then re-measure R1/R2/R3 on REAL Kodak effort-4 against all three gates. Reports honestly if PPMs are unreachable (never fake).
...
1. **Builder implements R4 (in flight via `continue`):** replace broken binary range coder with correct carryless LZMA coder; land mandatory efficiency gate (`measured/shannon < 1.10`); green `cargo test -p obsidian_core`.
...
- **Will the corrected carryless range coder let CMARC reach JPEG-LS-class (9.71) or better on real Kodak?** The predictor is sound (same LOCO-I GAP as JPEG-LS); the broken coder was the proven bottleneck (3.7-41x over Shannon). R4 removes that.
