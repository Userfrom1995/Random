...
- **Updated:** 2026-08-19 (~05:13Z, maintainer run 32218601716). **DECISIONS:** `[{"action":"continue","pr":83,"head":"7f636a45107675d77877e51e02f4b6248861360c"}]` - re-fire the Builder to fix the NEW gamma-overflow panic in `read_gamma` (`rans.rs:658`) as a hard stability gate FIRST, then the CMARC no-op/explosion integration bug, then re-measure real Kodak. No merge (gates unmet: 10.0906 bpp > WebP 9.61 / JXL 8.71, PNG 13.05 met). One PR preserved.
...
- **CMARC stack (R1 -> R2.4) + R3 + R4 built, all OFF by default (never-expand net).**
  ...
  - **NEW REGRESSION (2026-08-19 05:13Z): gamma-overflow panic in `read_gamma` (`rans.rs:658`).** The GR v1 (production default) path now panics: `(1u32 << k) | low` overflows / `read_bits(k)` when `k` runs away on a bad or corrupt bitstream. Likely a side effect of the R4 CACM87 rework in `rans.rs`. A default encode/decode of real Kodak crashes -> release-blocking. The Builder must confirm whether the crash is in the GR v1 default or only via the `GR_LZ` fallback, and fix both.
  - **OPEN DEFECT 1 (CMARC still a no-op / explodes):** with `OBSIDIAN_CARC_FORCE=1` the encoder output is byte-identical to GR on all 24 Kodak images, and/or the never-expand net falls back to `GR_LZ`. Root cause is integration-level: `code_planes`/`cmarc_*` routing, per-bin `BinModel` adaptation (prior/step/sparsity), or residual mapping/overflow - NOT the coder. CMARC still cannot beat GR on real Kodak.
...
- **Builder (resumed via `continue` this run, PR #83, head `7f636a4`):** RE-ORDERED priority: (1) **fix the `read_gamma` gamma-overflow panic as a HARD stability gate** - guard/clamp `k`, reject runaway codes with `CodecError::Corrupt`, verify GR v1 default path crash-free on all 24 Kodak + synthetic suite; confirm whether crash is in GR v1 default or only `GR_LZ` and fix both; (2) **fix CMARC no-op / explosion** (routing/per-bin `BinModel`/residual mapping) so CMARC emits its own bitstream; (3) **re-measure real Kodak effort-4** with fixed CMARC + record CSV; (4) **correct stale `progress/68-...md`** (data/kodak present; real Kodak = 10.0906). The harness auto-commits/pushes.
  - NOTE: the prior `continue` (owner 05:13:03Z) completed WITHOUT advancing the branch (head still `7f636a4`); it only posted the gamma-overflow finding. This run's `continue` re-fires the integration/stability work.
...
1. **Builder `continue` (re-fired this run):** fix gamma-overflow panic (hard gate) -> fix CMARC no-op/explosion -> re-measure real Kodak -> correct stale progress file.
...
- **Is the `read_gamma` overflow reachable from the GR v1 default path, or only via `GR_LZ`?** Builder must confirm and fix both; if only via `GR_LZ`, the GR v1 default crash is a different defect still needing fix.
- **Will the re-fired Builder actually advance the branch this time?** The prior `continue` (05:13:03Z) completed without pushing. Watch for a new opencode build run on `opencode/issue68-20260818070512` that lands the gamma-overflow fix + CMARC integration fix and a real-Kodak CSV.
