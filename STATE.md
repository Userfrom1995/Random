...
- **Updated:** 2026-08-22 (maintainer run 32540394080, EVENT `created`/duplicate `/oc maintainer` on PR #118, owner ~00:27Z). Follow-up pass confirming the prior run 32540388074 state.
- **PR #118 (Prism M1-M4) is the active priority and is IN FLIGHT.** Branch `opencode/117-prism-m1-m4-optimization` = `f5faad7cf2e6ad7ddbb07d78eb106b97d6eb8069` (B5.17, 11.120 bpp, byte-exact, MERGEABLE, shares `main` ancestry NOT orphan). A `continue` build `32540331936` is genuinely `in_progress` (started 00:26:06Z, confirmed active ~00:32Z at ~6 min, driving B6-B8). The duplicate owner `/oc continue` (00:27:14Z) spawned pending run `32540394102`, queued behind it (cancel-in-progress false).
...
- **`lab` PATH IS BROKEN (critical finding, unchanged):** `opencode.yml` has NO `lab` job (only research/architect/build/fix/general) and there is NO `opencode-lab.yml` workflow file. Therefore `/oc lab` (and `/oc auditor`) comments produce only SKIPPED runs (seen as `32540394100`/`32540388073`, conclusion=skipped). This means The Lab Engineer CANNOT be dispatched via `/oc lab` right now - so the silent-stall root-cause fix escalated in run 32540175107 was a NO-OP. The maintainer model-fallback policy restricts direct workflow edits to model switching only, so Mae cannot wire the `lab` job herself; this needs the owner to add the job (or authorize it). Short-term mitigation: Mae re-dispatches `continue` on detecting a stall.
