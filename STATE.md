...
- **Updated:** 2026-08-22 (maintainer run 32543666243, EVENT `schedule` ~01:31Z). Fresh survey confirms: PR #118 has a genuine `continue` build IN FLIGHT (`32543408136`, head `933276f`, B5.19, 11.119 bpp). #68 and #98 now CLOSED. PR #119 is stale/conflicting (target #98 already closed; workflow-file push wall). No merge (M3 < 8.71 unmet).
- **PR #118 (Prism M1-M4) REMAINS the active priority.** Branch `opencode/117-prism-m1-m4-optimization` = head `933276f16e5a1a7f156d910fce5acdd5d5a9f3de` (B5.19, 11.119 bpp, byte-exact, MERGEABLE, shares `main` ancestry NOT orphan). As of this run a `continue` build IS in flight (run `32543408136`, `in_progress`, started 01:26:01Z). A second owner `/oc continue` (01:26:11Z) is `pending` (`32543417553`) - queued behind with `cancel-in-progress: false`, harmless. No duplicate `continue` dispatched. M3 < 8.71 bpp gate firmly UNMET -> NO merge.
...
- **`main` = `02c0fb556d50be4ea056a734da7957420e9357b5`** (post PR #116 merge). Obsidian lives in `obsidian/` on `main`. Prism branch `opencode/117-prism-m1-m4-optimization` = `933276f` shares M0 ancestry (NOT orphan).
...
- **`lab` PATH IS STILL BROKEN (critical finding, unchanged):** `opencode.yml` has NO `lab` job (only research/architect/build/fix/general) and there is NO `opencode-lab.yml` workflow file. Therefore `/oc lab` (and `/oc auditor`) comments produce only SKIPPED runs, so The Lab Engineer CANNOT be dispatched via `/oc lab` - the silent-stall root-cause fix remains a NO-OP, and no bot can self-heal workflow-file PRs either. The maintainer model-fallback policy restricts direct workflow edits to model switching only, so Mae cannot wire the `lab` job herself; this needs the owner to add the job (or authorize it).
- **WORKFLOW-FILE PUSH WALL (unchanged):** the lab's GitHub App lacks the `workflows` scope, so pushes that touch `.github/workflows/*.yml` are rejected at push. PR #119 proves this (multiple `/oc fix` attempts `remote rejected ... without workflows permission`). Consequence: any future workflow-file PR cannot be fixed by the Fixer/Lab Engineer; the owner must grant `workflows: write` or apply the change manually.

## IN FLIGHT
- **Prism M1-M4 (issue #117, PR #118, branch `opencode/117-prism-m1-m4-optimization`):** optimization loop. Head `933276f` (B5.19, 11.119 bpp, byte-exact, harness 148s). As of run 32543666243 a `continue` build IS in flight (run `32543408136`, `in_progress`, started 01:26:01Z). A duplicate owner `/oc continue` (01:26:11Z) is `pending` (`32543417553`) - queued behind, harmless. No duplicate `continue` dispatched.
...
- **PR #119 (`[Infra] Lab update for #70`) - STALE / OWNER ESCALATION (NOT in flight by a bot).** Branch `opencode/lab-98-runaway-fix-retry`, head `eac12c1`, `mergeable=CONFLICTING`/`DIRTY`. Its body says `Closes #70` but the actual fix targets #98, which is NOW CLOSED (fix landed via PR #99 + Lab Engineer run `32540682703`). So the PR's premise is largely resolved in `main`; what remains is a conflicting workflow-file delta the bot cannot push (workflows-scope wall). Owner action required: rebase if any delta still needed, or close as redundant.

## PENDING (in order)
1. **Prism M1-M4 (PR #118):** a `continue` build IS in flight (run `32543408136`, head `933276f`, B5.19, 11.119 bpp) from the owner's `/oc continue` at 01:26:01Z; it resumes B6-B8 toward M3 < 8.71 bpp on real Kodak bit-exactly; then Reviewer -> Tester (real Kodak, bit-exact, bpp gates M1<13.05 & <9.61, M2<9.71, M3<8.71). HOLD merge until M3 met bit-exactly per owner override. Do NOT dispatch a second `continue` while one is active.
2. **PR #119 OWNER ESCALATION (UPDATED - now stale):** #98 is CLOSED; the branch is CONFLICTING and carries a workflow-file change the bot cannot push (workflows permission wall). Owner action: rebase #119 if any delta is still needed vs current `main`, or close it as redundant. The substantive #98 pagination/API-refusal fix already landed in `main` (PR #99 + run `32540682703`). Also resolve the `Closes` target mismatch (body says #70, branch/commits say #98).
3. **WIRE THE `lab` JOB (CRITICAL BLOCKER):** opencode.yml has no `lab` job and `opencode-lab.yml` does not exist, so `/oc lab` is a no-op. The owner must add a `lab` job to opencode.yml (modeled on the build job but running the Lab Engineer / LAB mode) so The Lab Engineer can run and fix the silent-stall dispatch logic AND future workflow-file PRs. Until then, the silent-stall root cause cannot be fixed by the Lab Engineer, and Mae's only mitigation is re-dispatching `continue` on detection.
4. **Silent-stall diagnosis (BLOCKED by #3):** owner `/oc continue` produced skipped/cancelled opencode runs FIVE times earlier (23:16/23:58/00:23Z stalls; plus the prior pattern). At 01:26:01Z the owner's `/oc continue` THIS TIME launched a real build (`32543408136`, in_progress) - the pattern is intermittent, not a hard block. The underlying opencode.yml issue_comment dispatch bug remains undiagnosed because the Lab Engineer cannot run (no `lab` job). Keep re-dispatching `continue` as the mitigation when a stall is detected; do NOT re-issue `/oc lab` (confirmed no-op, only adds skipped-run noise).
...
## NEXT STEPS
1. Prism M1-M4 (PR #118): build `32543408136` (head `933276f`, B5.19, 11.119 bpp) is in flight from the owner's `/oc continue` 01:26:01Z, resuming B6-B8 toward M3 < 8.71 bpp on real Kodak bit-exactly. Then Reviewer -> Tester (real Kodak, bit-exact, bpp gates); HOLD merge until M3 met bit-exactly per owner override. Do not dispatch a duplicate `continue`.
2. **PR #119 owner escalation (now stale):** #98 is closed; rebase or close the conflicting workflow-file PR. The #98 fix content already landed in `main`. Owner may also grant `workflows: write` so future such PRs self-heal.
...
## OPEN QUESTIONS
- Prism #118: will the in-flight `continue` (`32543408136`) iterate past ~11.119 bpp (B5.19) toward M3 < 8.71 on REAL Kodak bit-exactly? Owner override: no merge until M0+M1+M2+M3 met bit-exactly.
- Prism #118: when stable at/under the gate, fire Reviewer -> Tester before any merge.
- **PR #119:** #98 is now CLOSED. Should the owner rebase #119 (if any delta remains vs `main`) or close it as redundant? The `Closes` mismatch (#70 vs #98) is moot in substance now.
- **`lab` PATH BROKEN:** opencode.yml has no `lab` job and `opencode-lab.yml` does not exist, so `/oc lab` no-ops. Will the owner wire the `lab` job? Mae's only mitigation for the silent-stall is re-dispatching `continue` (and this round the owner's own `/oc continue` launched successfully).
- **WORKFLOW-FILE PUSH WALL:** the App lacks `workflows` scope; will the owner grant it so workflow-file PRs can self-heal, or continue to apply such changes manually?
- Silent-stall root cause: owner `/oc continue` at 23:16 / 23:58 / 00:23Z all failed to launch a build (skipped/cancelled); at 01:26:01Z it DID launch `32543408136`. Intermittent. Needs the Lab Engineer, which is currently unreachable. Mitigation: re-dispatch `continue` on detected stall.
