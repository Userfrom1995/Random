...
- **Updated:** 2026-08-19 (~15:30Z, maintainer run 32269425194, owner event on PR #83). **DECISIONS:** `architect` on PR #83 - design the transform-level breakthrough (palette + Squeeze transform, JPEG XL-class color/transform pipeline) the Builder escalated for, since R9-A/B/C structural features are all inert on photographic Kodak. No duplicate trigger (no Builder/Architect/Researcher in flight; R9-C build confirmed landed at head `644affe`). Default codec now 9.6678 bpp (PNG MET; WebP MISSED by +0.058; JPEG XL MISSED by +0.958). WebP gate is now within a whisker.
...
- **Mergeability (BROKEN):** `main` = `8f4c15b` (single orphan commit, no parent). Branch root = `75e2eaa` ("builder: rebuild Obsidian codec crate...", orphan) + commits -> head `644affe` (R9-C). `git merge-base origin/main <branch>` is EMPTY. GitHub reports `CONFLICTING`; `gh pr merge --rebase` would fail. `--rebase` is impossible until the Factory re-links.
...
- **M1 OPEN as PR #83** (single canonical PR, branch `opencode/issue68-20260818070512`, head `644affe`):
  ...
  - **KEY DIAGNOSIS (empirical, settled):** the codec is pinned at the **~9.6678 bpp ceiling** - a hair above JPEG-LS (9.71) but ~0.05 below the original 10.16. The entropy backend (CMARC, R4-corrected LZMA carryless range coder verified at `H(p)+epsilon`; R5 Rice quotient) is NOT the bottleneck, and the predictor bank (R9-B weighted tree) has also saturated. Remaining gaps are **transform-level / inter-channel + long-range structural redundancy**, which the R9-A/B/C structural features cannot reach. The Builder escalated: "The remaining WebP (+0.058) and JPEG XL (+0.958) gates need a genuinely different mechanism (palette/squeeze transform, or a JPEG XL-class color/transform pipeline)."
- **CMARC lineage (R1 -> R5) built; entropy core correct (CACM87 / LZMA range coder):**
  ...
  - **R5 (CMARC Rice quotient fix):** per-run-position adaptive `BinModel` learns the geometric quotient like JPEG-LS QM; delivered the headline numbers (R8-A/R9-B re-implemented cleanly on top).
  - All CMARC variants ship behind the never-expand safety net, which now ALSO engages by default.

## In flight

- **PR #83 (Obsidian, branch `opencode/issue68-20260818070512`, head `644affe`):** R9-A (spatial LZ77 2D-distance) DONE + DORMANT (0/24 LZ); R9-B (context-tree weighted predictor) DONE, +0.04 bpp help (9.7080 -> 9.6678); R9-C (run mode) DONE + INERT (9.6678). **This run fires `architect`** to design the transform-level breakthrough (palette + Squeeze transform, JPEG XL-class color/transform pipeline) the Builder escalated for. One-PR rule intact. Orphan-main break + unmet gates still apply. `mergeable: CONFLICTING`.
...
1. **PR #83 `architect` (this run):** Architect blueprints the transform-level breakthrough on the single PR - palette + Squeeze (channel-reorder/decorrelation) transform and/or a JPEG XL-class color/transform pipeline (color cache, proper YCoCg, Squeeze-style subpixel interleaving) - targeting WebP 9.61 (close) and JPEG XL 8.71. After the blueprint, this run's decision will resume the Builder via `continue`.
...
- **Orphan-main break (recurrence):** `main` = `8f4c15b` orphan; branch = `75e2eaa` orphan -> `644affe`. `git merge-base` empty. Factory must re-link AND fix the recurrence root cause; today the App cannot push main. Non-blocking now (gate unmet) but must be fixed before merge.
