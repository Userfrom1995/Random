...
**Gate reality (real 24-image Kodak, effort 4):** PNG 13.05 **MET**; WebP 9.61 **NOT MET** (+0.11 bpp); JPEG XL 8.71 **NOT MET** (+0.81 bpp). R13-A was the last blueprint-prescribed predictor lever and it does not help, confirming the ~9.52 bpp plateau is the ceiling of the current 4-tap linear predictor + context model on this corpus (consistent with the R6-B empirical ceiling and the JPEG-LS-floor analysis). CMARC already reaches `H(p)+epsilon`, so the gap to WebP/JPEG XL is a *predictor/context-model* gap, not a coder gap.
