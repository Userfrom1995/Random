...
**Gate reality (real 24-image Kodak, effort 4):** PNG 13.05 **MET**; WebP 9.61 **MET** (9.5209 < 9.61, -0.09 bpp); JPEG XL 8.71 **NOT MET** (+0.81 bpp). R13-A was the last blueprint-prescribed predictor lever and it does not help, confirming the ~9.52 bpp plateau is the ceiling of the current 4-tap linear predictor + context model on this corpus (consistent with the R6-B empirical ceiling and the JPEG-LS-floor analysis). CMARC already reaches `H(p)+epsilon`, so the remaining gap is the JPEG XL *predictor/context-model* gap, not a coder gap.

**Next phase (decision = `continue`):** the blueprint's build order (R13-B lifting only if R13-A reaches ~8.8-9.0) is not satisfied by R13-A, and R13-B (CDF 5/3 lifting) is a decorrelation transform unlikely to bridge the ~0.8 bpp gap alone. The remaining path to the gate is an **R7-class predictor/context-model effort** (adaptive weighted predictor + larger/MA context tree, the WebP/JPEG XL-class pipeline). R13-A infrastructure (the `AdaptiveRecursive` predictor, the `forced_predictor` seam, the lockstep tests) is retained as research capability and as the per-band R13-A component of a future R13-B. Per owner override, no merge until PNG + WebP + JPEG XL are all beaten bit-exactly. Action = `continue`.
