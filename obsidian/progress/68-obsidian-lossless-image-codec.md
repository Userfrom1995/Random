...
- [ ] 36. **R13-A (Architect blueprint 2026-08-20): recursive self-correcting adaptive multi-tap predictor (TM-WP class).** `predict.rs`: add `PredictorId::AdaptiveRecursive = 19` (bump `PREDICTOR_COUNT` to 20), `R13_M = 9`, `R13_SHIFT`/`R13_SCALED_B`, `r13_properties`/`predict_recursive`/`adapt_recursive`/`solve_r13_least_squares`. `model.rs`: `weighted_r13_table`, per-`weight_context`-leaf `(M+1)x(M+1)` normal equations in `analyze`/`analyze_bands`, `r13_leaf_for`. `encoder.rs`/`decoder.rs`: per-context `wrstate[cid]`, unconditional `predict_recursive`+`adapt_recursive` when the map selects `AdaptiveRecursive`; **remove the broken `OBSIDIAN_M3_WP` seam** and its two tests, replace with `r13_*` lockstep tests. Strict superset of R9-B/GAP (base-init from R9-B machinery). **Target: real Kodak effort-4 < 8.71 (JPEG XL).** Record `benchmarks/results/2026-08-20-r13a-recursive-adaptive.csv`.
- [ ] 37. **R13-B (additive, only if R13-A lands ~8.8-9.0): genuine CDF 5/3 lifting wavelet + per-band R13-A.** `transforms.rs`: `cdf53_lift`/`cdf53_unlift` (separable, symmetric-clamp border, reuses `squeeze_band_layout` geometry); then R13-A applied per-band on LL (the corrected, now-non-inert R12-A). Expected ~9.0-9.3 alone; with R13-A the complement that closes 8.71. Record `benchmarks/results/2026-08-20-r13b-lifting.csv`.

## Current step
**BUILDER STATUS (2026-08-19, /oc continue run):** R4+R5 landed (CMARC 9.7579 bpp on real Kodak, at the JPEG-LS floor). Component B (R3-A residual-context) is DONE and verified: real Kodak CMARC+R3-A = **9.7067 bpp** (clears JPEG-LS 9.71; per-image result in `benchmarks/results/2026-08-19-r3a-quotient-context.csv`). R6-B color cache was the corrected blueprint's prescribed primary WebP (9.61) lever; it is now **IMPLEMENTED and MEASURED net-negative** on photographic Kodak (forced size-32 = 12.88 bpp vs plain 9.7093; the safety net correctly never selects it). Closed-form analysis: the cache only beats the residual at an exact-value hit rate > ~76%, which natural photographs do not provide at any cache size (small cache -> too few hits, large cache -> prohibitive index cost). So R6-B is NOT the WebP lever for this architecture.

**Gate reality (real 24-image Kodak, effort 4):** PNG 13.05 **MET**; JPEG-LS 9.71 **MET** (9.7067); **WebP 9.61 NOT MET** (+0.10 bpp); **JPEG XL 8.71 NOT MET** (+1.00 bpp). Every CMARC-stage extension (R1-R5, R2.1-R2.4, M2/M2.5/M3-A/M3-B/M3.5, R6-B) has now been built and measured on the real corpus; the codec plateaus at **~9.71 bpp = the JPEG-LS floor on the same LOCO-I GAP predictor**. This is a genuine architectural ceiling of the current predictor + context model, not a coder bug: CMARC already reaches `H(p)+epsilon`, and JPEG-LS itself (same predictor, QM coder) is also at 9.71, while WebP (9.61) / JPEG XL (8.71) win via a *better predictor* (WebP 4-mode + adaptive weighted; JPEG XL MA-tree + adaptive weighted) and a richer context model.

**Next phase (escalated to Maintainer/Architect):** an **R7 predictor/context-model effort** (adaptive weighted predictor + larger/MA context tree, the WebP/JPEG XL-class pipeline the Architect flagged as possibly-needed in the R6 blueprint) is required to clear the WebP/JPEG XL gates. Incremental CMARC extensions (R6-C tuned matches, stated "marginal ~0.05-0.1 bpp", insufficient alone) are exhausted. Production codec unchanged at 9.71 bpp (PNG gate MET); per owner override, no merge until PNG + WebP + JPEG XL are all beaten bit-exactly. Decision recorded: `docs/decisions/builder/2026-08-19-r6b-colorcache-empirical-ceiling.md`; action = `maintainer`.
