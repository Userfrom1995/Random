...
- **Issue:** #68
- **Author:** Dr. Ada, the Researcher
- **Date:** 2026-08-16

This document is the literature review and quantitative landscape for the
Obsidian codec. The companion documents are the algorithmic specification
(`docs/algorithmic-spec.md`) and the benchmark protocol (`docs/benchmark-methodology.md`).
...
mixing) is documented as a stretch goal and as a lower-bound reference, not as
the primary target.
...
### 2.1 The pipeline that every modern lossless codec shares
...
3. **Context modeling**: partition residuals into contexts such that the
...
we get to the entropy of the conditioned residual stream. This is the
canonical taxonomy, stated repeatedly in the JPEG-LS (LOCO-I), CALIC, and
JPEG XL literature.
...
- **GAP (gradient-adjusted predictor)**, from CALIC: a threshold-controlled
  blend of left/top/diagonal values based on local gradient magnitudes.
- **Multiple predictors per pixel**: JPEG XL modular mode combines a bank of
  ...
  (an ensemble of up to four sub-predictors whose weights and max-error bookkeeping
  adapt to the local region). Predictor selection is context-driven, on a
  per-pixel basis. This is the key reason JPEG XL lossless leads the practical
  codecs.
...
- Channel-wise prediction with cross-channel prediction (predict chroma from
  luma) is another option; keep as a candidate.

### 2.4 Context modeling

- **JPEG-LS**: quantizes three local gradients `(a-b, b-c, c-d)` into a
  bounded set of contexts with a symmetry-reduction mapping, then codes the
  residual with a context-adaptive Golomb-Rice coder.
- **FLIF (Free Lossless Image Format)**: *MANIAC* (Meta-Adaptive Near-zero
  ...
  stream. Interlaced coding (encode a low-resolution version first, refine
  with residuals) gives FLIF very strong contexts.
- **JPEG XL modular**: combines a **meta-adaptive (MA) tree** (a signaled
  decision tree over "properties" such as neighborhood sample values and the
  predictor's max_error) with **static entropy distributions** learned in an
  encoding pass. The MA tree selects both the predictor and the context per
  pixel. Histogram sharing between tree leaves avoids redundant tables.
...
- **ANS - asymmetric numeral systems** (JPEG XL rANS, plus tANS variants in
  ...
  sweet spot: *rANS with static tables learned per context* is what makes
  JPEG XL lossless both compact and fast.
...
| FLIF | interpolating + MANIAC tree | MANIAC (meta-adaptive) | adaptive arithmetic | SOTA 2016-2019, superseded by JXL |
...
Independent aggregate benchmark (WangXuan95/Image-Compression-Benchmark, 2024,
across CLIC2021+LPCB+GDCC2020+UCID+ImgInfo+GDCC datasets): JPEG XL (-q100 -e5)
is the smallest; WebP lossless (m5) is ~7.5% larger than JPEG XL; optipng PNG
is ~28% larger than JPEG XL; JPEG-LS is ~29% larger. JPEG XL's lead is
consistent with the per-image Kodak literature above.
...
predictor + MA-tree) and rANS entropy coding over per-context static tables.
