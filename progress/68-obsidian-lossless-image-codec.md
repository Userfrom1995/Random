...
## Current step
**ARCHITECT R7 BLUEPRINT DELIVERED (2026-08-19, the Architect, on PR #83) - per-context least-squares weighted predictor.** The codec plateaued at **9.7093 bpp = the JPEG-LS floor** (CMARC + R3-A + R2.1 subtract-green). The entropy backend (CMARC, R4 correct coder) is verified at `H(p)+epsilon`, so the residual structure is **prediction error** and cannot be coded away - the bottleneck is the **predictor**, exactly as the Builder's ceiling analysis (`docs/decisions/builder/2026-08-19-r6b-colorcache-empirical-ceiling.md`) concluded. `predict.rs` already provides `WeightVec` + `predict(Weighted, n, w)` + a 16-vector `default_weight_codebook()`, and `analyze` picks a single **per-plane** weight; R7 upgrades this to a **per-context least-squares optimal weight**, signaled in the model section with zero online state (so encoder/decoder lockstep is exact - the precise failure mode of the regressed M3-B online SGD). The per-context best-of-codebook is a strict superset of the current single per-plane weight, so it provably cannot raise residual energy. Blueprint: `obsidian/docs/architect-r7-weighted-predictor-blueprint.md`. Build order: R7-A (per-context LS weights, target <= 9.61 WebP) -> R7-B (predictor class in residual context) -> R7-C/D (re-enable LZ77, Components C/D of R6) -> R7-E (MA-tree context / adaptive per-pixel weighted, stretch toward JXL 8.71). Gates: PNG 13.05 MET; JPEG-LS 9.71 MET (9.7067); WebP 9.61 target of R7-A (realistic, weighted-predictor win well established); JPEG XL 8.71 target of R7-A+B+C/D, **UNCERTAIN** (may need follow-on R8). Decision: `continue` (Builder resumes R7-A on this branch; Factory keeps `data/kodak` committed so the gates stay measurable).
