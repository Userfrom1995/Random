...
- [ ] 47. **R10-C (stretch, only if R10-A + R10-B > 8.71):** finer Squeeze interpolation (gradient-adaptive horizontal/vertical avg to shrink HF residuals further) and/or fold the R9-B weight-context into the CMARC quotient context for the LL band (MA-tree-style). Real Kodak = 9.5208 > 8.71, so R10-C is REQUIRED to clear the JPEG XL gate. Build only after R10-A/R10-B measured.
...
## Current step
**R10-A (Squeeze) + R10-B (CFL) IMPLEMENTED and measured on REAL Kodak (effort 4, 24-image `data/kodak`) = 9.5208 bpp** (`benchmarks/results/2026-08-19-r10.csv`). This is the biggest single-stage gain since R4 (-0.147 bpp vs R9-B 9.6678) and **clears the WebP 9.61 gate** (and JPEG-LS 9.71 and PNG 13.05). Both transforms are behind the never-expand safety net (plain / CFL / CFL+Squeeze compared, smallest kept) and are bit-exact (136 lib tests green). Squeeze reuses `code_planes`/`decode_planes` verbatim: each sub-band is an ordinary coding plane with its own `(w,h)` (threaded via new `dims`/`parent` params); CFL is a full-res luma pre-subtract composed with Squeeze. The remaining gate is **JPEG XL 8.71** (9.5208 vs 8.71, +0.81 bpp): R10-B's CFL gain was modest on natural Kodak (YCoCg-R already removed most chroma energy), so per the blueprint R10-C (finer Squeeze interpolation / MA-tree context on the LL band) is the named fallback and is now REQUIRED to clear JXL. **Decision: `continue`** - the Builder resumes R10-C on this single branch, re-measuring on durable `data/kodak`; the owner override forbids merge until PNG + WebP + JPEG XL are all beaten bit-exactly.
