...
## Current step
**BUILDER STATUS (2026-08-19, /oc continue run):** R4+R5 landed (CMARC 9.7579 bpp on real Kodak, at the JPEG-LS floor). Component B (R3-A residual-context) is now DONE and verified: real Kodak CMARC+R3-A = **9.7067 bpp** (clears JPEG-LS 9.71, no regression; per-image result in `benchmarks/results/2026-08-19-r3a-quotient-context.csv`). Net march order per the corrected R6 blueprint (Architect commit `f137881`, `docs/architect-r6-corrected-blueprint.md`): next is Component A = R6-B color cache (`ENTROPY_MODE_CARC_CACHE = 6`, item 32) as the primary WebP (9.61) lever; then Component C (tuned matches); Component D (multi-channel) deferred. The spatial LZ77 back-reference (old item 31) is DROPPED per the corrected blueprint.
