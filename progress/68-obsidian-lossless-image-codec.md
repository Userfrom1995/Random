...
- [ ] 31. **R6-A: pixel-domain spatial LZ77 (ARCHITECT R6 BLUEPRINT DELIVERED, 2026-08-19, the Architect, on PR #83).** Replace the residual-domain match layer with a **spatial back-reference over the reconstructed sample buffer** `recon[c]` (per-plane): each pixel is a LITERAL (predictor residual via CMARC/R5) or a MATCH that copies `recon[c][i-offset+j]` for `length` samples (no residual coded; predictor bypassed for matched pixels but the next literal sees the copied recon neighbors -> bit-exact by induction). New `ENTROPY_MODE_CARC_SPATIAL = 5` signalled in `model.entropy_mode` (no header flag bit); reuses `[carc_len u32 LE][carc_bytes]` framing. Match finder = M3-A `lz_*` keyed on **recon values** (WIN=min(width*2,32768), MIN_MATCH=3, MAX_MATCH=256, MAX_CHAIN=256). Token stream: `match_flag` (CMARC bin, gradient context) + offset (distance model) + length (Elias-gamma). Mirrored `use_spatial_lz` model flag; `analyze` codes plane twice, keeps smaller; global never-expand net preserved; default OFF (`OBSIDIAN_SPATIAL_LZ`). Re-measure real Kodak; **target < 9.61 (WebP)**. Record `benchmarks/results/2026-08-19-real-kodak-r6a.csv`.
- [ ] 32. **R6-B: color cache (LRU, per-plane, CACHE=512 signalled).** A LITERAL whose reconstructed value is in the LRU is coded as `cache_flag=1` + small index instead of the full residual; decoder maintains the identical LRU. Additive with R6-A; re-measure, target further toward 8.71 (JPEG XL). Record `...-r6b.csv`.
- [ ] 33. **Fix R3-A residual-context (verify it is real).** `cmarc-force+resctx` is currently byte-identical to `cmarc-force` -> the per-(dL,dU,dUl) context is inert (hard-coded id or dead branch). Add a trace asserting the context id is non-constant on Kodak and that `+resctx` then differs; fix the wiring so different contexts select different `BinModel`s. Free additive win once unstuck; do NOT ship R6 on top of a silently-broken R3-A.
