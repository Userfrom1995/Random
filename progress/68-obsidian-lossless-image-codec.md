...
## Current step
**R9 BLUEPRINT (context-tree weighted predictor + 2D spatial LZ) is the active track** (`obsidian/docs/architect-r9-spatial-lz-weighted-predictor.md`), superseding the R7 per-context weighted-predictor blueprint. The codec plateaued at **9.7093 bpp** (CMARC + R3-A + R2.1), at the JPEG-LS floor; the entropy backend is already at `H(p)+epsilon`, so the remaining gap is the **predictor** (R9-B) plus low-entropy run/2D structure (R9-C). Build order per the blueprint: **R9-A** (2D-distance CMARC-LZ) -> **R9-B** (context-tree weighted predictor, the JPEG XL / WebP lever) -> **R9-C** (run mode). R9-A was committed as dormant (LZ never selected on Kodak, 0/24 images; the residual-domain LZ77 ties). R9-B is now DONE and MEASURED this run (see below). Gates: PNG 13.05 MET; JPEG-LS 9.71 MET (9.7067); **WebP 9.61 = 9.6678 (R9-B, +0.058 bpp to clear, NOT YET MET)**; JPEG XL 8.71 OPEN (+0.958 bpp). R9-C (run mode, below) is now DONE and MEASURED: it is INERT on photographic Kodak (the safety net never selects it; forcing it regresses to 9.7175), so the residual-entropy floor after the weighted-tree predictor is ~9.6678 bpp. Run/LZ/cache structural features cannot beat the already-near-optimal per-pixel CMARC coder on Kodak. Decision: `maintainer` (the remaining WebP/JPEG XL gap needs a genuinely different mechanism - the R9-C stretch palette/squeeze transform, or a JPEG XL-class color/transform pipeline; the owner override forbids merge until all three gates are beaten bit-exactly).
