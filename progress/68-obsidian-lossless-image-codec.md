...
- [ ] 41. **R9-C: run mode for near-constant regions (WebP/JPEG XL final lever).** On contexts where `weight_context` / the local gradients are ~0 (flat regions), encode a run flag + Elias-gamma run length instead of a residual; decoder copies `prev_val`. Target: clear WebP 9.61 (expect ~9.4-9.6 bpp). Build after R9-B measures.
