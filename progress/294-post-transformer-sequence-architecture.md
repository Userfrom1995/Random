...
- **Current step:** M4n verification complete on torch CPU expectation (ledger 25 rows green, py_compile clean, scope clean, PR MERGEABLE per gh, zero forward_chunk refs, Refs #294; full pytest re-run rests on the torch-env Tester pass). Next: S-tiny full gates on a GPU runner.
