...
- A2 W{0,16,32} seed0: N8 0.275/0.287/0.251 - BUT the pre-fix G1 harness had no `--window` passthrough, so every eval ran window 16 (proven by `config.window=16` inside `g1_summary_p1-toy-W0/W32-s0.json`); no A2 conclusion may be drawn until re-eval with the fixed harness. Ledger rows renamed to valid `--model` values with the true train-window in the `window` column. A real A2 test needs S-tiny N64+.
